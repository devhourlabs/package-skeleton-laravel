...
```bash
php artisan vendor:publish --provider="Devhour\Skeleton\SkeletonServiceProvider" --tag="migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --provider="Devhour\Skeleton\SkeletonServiceProvider" --tag="config"
```
...
```php
$skeleton = new Devhour\Skeleton();
echo $skeleton->echoPhrase('Hello, DevHour!');
```
